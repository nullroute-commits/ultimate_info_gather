...
{_render_worker_services(plan)}
  ...
  authentik-postgres:
    ...
    entrypoint:
      ...
      - -c
      - |
        export POSTGRES_PASSWORD="$(cat /run/secrets/authentik_pg_password)"
        exec docker-entrypoint.sh postgres
    environment:
  ...
  hydra-postgres:
    ...
    entrypoint:
      ...
      - -c
      - |
        export POSTGRES_PASSWORD="$(cat /run/secrets/hydra_pg_password)"
        exec docker-entrypoint.sh postgres
    environment:
      POSTGRES_DB: hydra
      POSTGRES_USER: hydra
...
def render_populate_env_secrets_script() -> str:
    """Render a helper script that writes real secret values into env/diode.env.

    The diode-ingester and diode-reconciler images are distroless Go binaries
