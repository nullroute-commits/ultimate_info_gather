...
class PlannerTests(unittest.TestCase):
    ...
    def test_bundle_writer_emits_expected_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("diode-token-adapter:", compose_text)
            ...
            diode_adapter_py = (output_dir / "configuration" / "diode-token-adapter.py")
            self.assertTrue(diode_adapter_py.exists())
            adapter_text = diode_adapter_py.read_text(encoding="utf-8")
            ...
            hydra_env_text = (output_dir / "env" / "hydra.env").read_text(encoding="utf-8")
            self.assertIn('STRATEGIES_ACCESS_TOKEN=jwt', hydra_env_text)
            self.assertNotIn('STRATEGIES_JWT_SCOPE_CLAIM', hydra_env_text)
            hydra_yml_text = (output_dir / "configuration" / "hydra.yml").read_text(encoding="utf-8")
            self.assertIn('scope_claim: string', hydra_yml_text)
            self.assertIn('./configuration/hydra.yml:/etc/hydra.yml', compose_text)
            self.assertIn('-c /etc/hydra.yml', compose_text)
            ...
            self.assertIn('DIODE_AUTH_TOKEN_URL=http://diode-proxy:80/auth/token', diode_env_text)
            self.assertIn('DIODE_AUTH_INTROSPECT_URL=http://diode-auth:8080/introspect', diode_env_text)
            self.assertIn('NETBOX_DIODE_PLUGIN_API_BASE_URL=http://netbox:8080/api/plugins/diode', diode_env_text)
            self.assertIn(
