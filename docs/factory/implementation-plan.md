...
16. Generate a single `netbox-init` one-shot service that executes `scripts/netbox-init.sh`, which calls `sync-superuser.sh` (create bootstrap superuser, mint v2 API token, write full token to `token-store` volume) followed by `setup-diode-credential.sh` (create Diode admin user and API token). This replaces the former two-container chain (`netbox-superuser-sync` → `diode-credential-setup`), eliminating one `service_completed_successfully` hop and providing a single gate for all downstream init consumers (`geo-foss-import`, `device-type-library-import`).
